...
const fetcher = async (url: string): Promise<GetResp | null> => {
  const res = await fetch(url, { cache: 'no-store' });

  if (!res.ok) {
    // Keep UI stable; SWR will expose error state if needed
    throw new Error(`Failed to load assets (${res.status})`);
  }

  const j = (await res.json()) as GetResp;
  return j?.ok ? j : null;
};
...
export default function AssetsPage() {
  ...
  const swrKey = address
    ? `/api/assets?${new URLSearchParams({ wallet: address, network: 'mainnet' }).toString()}`
    : null;
  ...
  const {
    ...
    isLoading,
    error,
  } = useSWR<GetResp | null>(swrKey, fetcher, {
    revalidateOnFocus: false,
    keepPreviousData: true,
  });

  // Preserve old semantics:
  // - if no wallet => treat as "no data"
  // - if wallet exists and loading => show loaders
  const data = dataResp ?? null;
  ...
  // Loading logic for UI blocks
  const walletMissing = !address;
  const showLoading = !!address && (isLoading || (dataResp === undefined && !error));
  const showError = !!address && !!error;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-4 space-y-3">
        ...
        <p className="text-xs text-zinc-600">{t('assets.buy.subtitle')}</p>
        ...
        <div className="flex flex-wrap items-end gap-3 mt-2">
          <div>
            ...
            <input
              ...
              value={buyAmount}
              onChange={(e) =>
                setBuyAmount(Math.max(1, Math.floor(Number(e.target.value) || 1)))
              }
            />
          </div>

          <button type="button" onClick={buy} className="btn btn-outline text-sm">
            {t('assets.buy.btn')}
          </button>
      ...
      <div className="card overflow-hidden">
        <div className="px-4 pt-4 pb-2">
          <div className="text-sm font-medium">{t('assets.nft.title')}</div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {claimFromNft > 0 && (
                <tr className="border-t border-zinc-200">
                  <td className="px-4 py-2 text-zinc-700">
                    ...
                    <div className="text-xs opacity-70">{t('assets.vigri.from_nft')}</div>
                  </td>
                  <td className="px-4 py-2 text-zinc-700">{claimFromNft.toLocaleString()}</td>
                  <td className="px-4 py-2 text-zinc-700">
                  ...
                  <td className="px-4 py-2 text-zinc-700">{cf.format(vigriFromNftValueEur)}</td>
                </tr>
              )}
              ...
              <tr className="border-t border-zinc-200">
                ...
                <td className="px-4 py-2 text-zinc-700">{(0).toLocaleString()}</td>
                <td className="px-4 py-2 text-zinc-700">
                  {VIGRI_TGE_PRICE_EUR.toLocaleString('en-US', {
                    minimumFractionDigits: 4,
                    maximumFractionDigits: 4,
                ...
                <td className="px-4 py-2 text-zinc-700">{cf.format(0)}</td>
              </tr>
              ...
              {nonVigriPositions.map((p) => (
                <tr key={p.symbol} className="border-t border-zinc-200">
                  ...
                  <td className="px-4 py-2 text-zinc-700">{p.amount.toLocaleString()}</td>
                  <td className="px-4 py-2 text-zinc-700">{cf.format(p.priceEUR)}</td>
                  <td className="px-4 py-2 text-zinc-700">{cf.format(p.valueEUR)}</td>
                </tr>
              ))}
            </tbody>
            ...
            <tfoot>
              <tr className="border-t border-zinc-200 bg-zinc-50">
                ...
                <td className="px-4 py-2 font-semibold">{cf.format(displayTotalEur)}</td>
              </tr>
            </tfoot>
          </table>
        )}
      ...
      {/* History (separate component; key resets page without effects) */}
      <HistorySection
        key={`${address ?? 'no-wallet'}:${data?.history?.length ?? 0}`}
        t={t}
        address={address}
...
function HistorySection({
  ...
  const historyPageCount = history.length ? Math.ceil(history.length / HISTORY_PAGE_SIZE) : 0;
  ...
  const historyPageItems = history.length ? history.slice(start, start + HISTORY_PAGE_SIZE) : [];
  ...
  return (
    <div className="card p-4">
      ...
      {walletMissing ? (
        ...
        <div className="text-xs text-zinc-600 opacity-70">
          {t('overview.loading_history')} <span className="opacity-70">(error)</span>
        </div>
      ) : history.length ? (
        <>
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    >
                      {activityIcon(h.type)}{' '}
                      <span className="ml-1">{activityLabel(h.type)}</span>
                    </span>

                    <span className="truncate text-xs text-zinc-500">
                      {h.symbol} {h.amount > 0 ? `+${h.amount}` : h.amount}
                    </span>
                  </div>

                  {/* Line 2 */}
                  <div className="mt-0.5 flex items-center gap-2 text-xs text-zinc-500 min-w-0">
                    <span className="truncate">{new Date(h.ts).toLocaleString()}</span>
